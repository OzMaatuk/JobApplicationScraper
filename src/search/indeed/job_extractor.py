...
class JobExtractor:
    ...
    def job_url(self) -> Optional[str]:
        return get_element_attribute(self.page, self.constants.Locators.Job.URL, "data-jk")
    ...
    @property
    def is_job_easy_apply(self, parent_element) -> bool:
        try:
            wait_for_element(parent_element, self.constants.Locators.Job.EASY_APPLY)
            return True
        except Exception:
            return False
    ...
    def _process_job_elements(self, limit: Optional[int] = None) -> List[Job]:
        ...
        for element in elements:
            try:
                element.click()
                job = self._process_single_job_element(element)
                if job: jobs.append(job)

                processed_elements += 1
                if processed_elements >= limit:
                    logger.info(f"Processed {processed_elements} job elements. Stopping.")
                    break
            except Exception as e:
    ...
    def _process_single_job_element(self, element) -> Optional[Job]:
    ...
    def _extract_single_job_data(self, element) -> Dict:
        ...
        job = {
            ...
            "easy_apply": self.is_job_easy_apply(element),
        }
